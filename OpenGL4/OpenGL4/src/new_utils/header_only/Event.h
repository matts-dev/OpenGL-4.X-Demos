...
{	
	...
	*/
	...
	// Be careful, Strong subscribers cannot be cleaned up in the dtor class types because the strong object itself
	...
	struct StrongAgnosticSubscriber : public Subscriber<Args...>
	{
		sp<IEventSubscriber> strongObj;
		StrongAgnosticSubscriber(const sp<IEventSubscriber>& obj) : strongObj(obj) {}
	};
	...
	{
		static_assert(std::is_base_of<IEventSubscriber, T>::value, "Smart Subscribers must be IEventSubscriber Objects.");
	...
	/**
		Event allows multiple objects to subscribe to the delegate's event and be notified.
		Event is built for single threaded usage.
	*/
	template<typename... Args>
	class Event
	{
	public:
		...
		*/
		...
		void addWeakObj(const sp<T>& obj, void(T::*fptr)(Args...))
		{
			...
			if (!broadcasting)
			{
				IEventSubscriber* objAddress = obj.get();
		...
		*/
		...
		void removeAll(const sp<T>& obj)
		{
			...
			else
			{
				...
				QueuedRemoveAlls.emplace_back(std::static_pointer_cast<IEventSubscriber>(obj));
			}
		}
		...
		void completeQueuedOperations()
		{
			...
			for (const sp<IEventSubscriber>& entity : QueuedRemoveAlls)
			{
				//remove all this
				removeAll(entity);
			}
			...
			{
				IEventSubscriber* objAddress = newStrongSub->strongObj.get();
			...
			{
				if (sp<IEventSubscriber> obj = newWeakSub->weakObj.lock())
				{
		...
		bool hasBoundStrong(const IEventSubscriber& const_obj)
		{
			...
			IEventSubscriber* obj = const_cast<IEventSubscriber*>(&const_obj);
...
/** Use smart pointer managed delegates to achieve pass-able delegates; or define the macro for copy ctors */
		...
		Event<Args...>& operator=(Event<Args...>&& move) = delete;
	...
	private: //these are explicitly ordered for appearance in debugger!
		...
		std::multimap< IEventSubscriber*, sp<StrongAgnosticSubscriber<Args...>> > strongSubscribers;
		std::multimap< IEventSubscriber*, sp<WeakAgnosticSubscriber<Args...>> > weakSubscribers;
		...
		std::vector<sp<IEventSubscriber>> QueuedRemoveAlls;
